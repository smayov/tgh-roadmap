...
// Replica exactamente hashearPin(pin, salHex) del alta de empleados, pero con el módulo crypto de Node
...
function pinCoincide(pinIntroducido, pinHashGuardado) {
  ...
  const hashCalculado = calcularHash(pinIntroducido, salHex);
  return hashCalculado === hashEsperado;
}

export async function ficharConPin(negocioId, pin, ubicacion) {
  // 1. ¿Está bloqueado este negocio por intentos fallidos?
  ...
  // 2. Buscar empleado cuyo PIN coincida (activo = true)
  ...
  if (!empleado) {
    const nuevosIntentos = (bloqueo?.intentos_fallidos ?? 0) + 1;
  ...
  // 4. Decidir automáticamente Entrada o Salida mirando el último fichaje de hoy
