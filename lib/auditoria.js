...
// --- Guardado de la auditoría completa (8 pasos) ---

export async function guardarAuditoria(userId, auditoriaId, payload) {
  const admin = supabaseAdmin();
  ...
  if (auditoriaId) {
    // Ya existe: actualizamos, comprobando propiedad a mano (el cliente admin salta RLS)
    const { data, error } = await admin
      .from('auditorias')
      .update(campos)
      .eq('id', auditoriaId)
