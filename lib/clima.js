...
const TRADUCCION_TAGS = {
  'Extreme high temperature': 'Temperatura extrema alta',
  'Extreme low temperature': 'Temperatura extrema baja',
  'Heavy rain': 'Lluvia intensa',
  'Thunderstorm': 'Tormenta',
  'Strong wind': 'Viento fuerte',
  'Coastal event': 'Aviso costero',
  'Flood': 'Inundación',
...
function traducirTag(tag) {
  return TRADUCCION_TAGS[tag] || tag;
}
export async function obtenerClimaNegocio(lat, lon) {
  ...
  try {
    ...
    const descEs = Array.isArray(detalle.description)
            ? detalle.description.find((d) => d.language === 'es-ES')?.description
            : detalle.description;

          alertas.push({
            origen: detalle.sender_name,
            evento: detalle.event?.trim() || traducirTag(detalle.tags?.[0]) || 'Aviso meteorológico',
            inicio: detalle.start,
            fin: detalle.end,
            descripcion: descEs || 'Sin descripción disponible.',
          });
        }
      } catch {
